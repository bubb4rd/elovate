...
| 2026-09-16 | Season 06 Ranked SR-economy update: deployment fees increase in Diamond and above (SR division breakpoints unchanged); Diamond fees 75/85/95 (was 65/70/75), Crimson fees 130/135/140 (was 85/95/110), Iridescent/Top 250 step table now tops out at 270 (was capped at 220), stepping irregularly rather than a fixed +10 every 500 SR. `src/lib/ranked/ranks.ts` is now season-keyed (`SCHEDULE_PRE_S06` / `SCHEDULE_S06`, resolved via `srScheduleForSeason()`) so historical replay of pre-S06 seasons still reproduces the old fee values; `rankFromSr()`/`deploymentFee()` take an optional trailing schedule param, default the active (S06) schedule, and existing call sites are unmodified. Also fixed a pre-existing bug where Crimson's tier breakpoints were interpolated evenly (8333/9167) instead of the official 8300/9100 — now pinned explicitly for both schedules. OCR fee validation (`parse-sr-breakdown.ts`) now accepts the union of fees from both schedules so old screenshots still parse. SR Challenges were removed in S06; this codebase never modeled that feature, so there's nothing to change. |
